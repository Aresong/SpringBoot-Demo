package com.s5.security.config;
...
//        super.configure(http);
        ...
        //开启自动配置的登录功能。效果：如果没有权限则会来到登录页面
        http.formLogin().usernameParameter("user").passwordParameter("pwd")
                .loginPage("/userlogin");
        //1./login来到登录页
        //2.重定向到/login?error表示登录失败
        //3.更多详细规则
        //4.默认post形式/login代表处理登陆
        //5.一旦定制loginPage：那么loginPage的post请求就是登录

        //开启自动配置的注销功能
        http.logout().logoutSuccessUrl("/");//注销成功后来到首页
        //1.访问/logout，表示用户注销，清空session
        //2.注销成功后返回/login?logout页面

        //开启记住我功能
        http.rememberMe().rememberMeParameter("remeber");
        //登陆成功以后，将cookie发给浏览器保存，以后登录带上这个cookie，只要通过检查就可以免登陆
        //如果点击注销就会自动删除cookie
    }
